    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            ...
            for (Method declaredMethod : declaredMethods) {
                if (declaredMethod.getName().equals(route.getAction())) {
                    if (method == null) {
                        method = declaredMethod;
                    }
                    else {
                        throw new ServletException("Too many methods");
                    }
                }
            }
        }
        ...
        for (Object parameter : method.getParameterTypes()) {
            if (parameter == Map.class) {
                parameters.add(view);
            }
            else if (parameter == HttpServletRequest.class) {
                parameters.add(request);
            }
            else {
                throw new ServletException("Wrong method");
            }
        }
